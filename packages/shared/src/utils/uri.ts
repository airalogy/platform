...
export function getURLFromUri(url: string, protocol = "https:") {
  const hasProtocol = !url.split("://")[0].includes(".")
  if (hasProtocol) {
    return new URL(url)
  }
...
export function parseUri(uri: string): UriComponents {
  try {
    ...
    // Handle non-standard URIs (like custom schemes)
...
export function serializeUri(components: UriComponents): string {
  return `${components.scheme}:${components.path}`
}
...
*/
export function getUriPathBasename(uri: string, isPath?: boolean): string {
  const path = isPath ? uri : getCleanUriPath(uri)
...
export function joinPathsToUri(uri: string, ...pathSegments: string[]) {
  ...
  return serializeUri(components)
}
