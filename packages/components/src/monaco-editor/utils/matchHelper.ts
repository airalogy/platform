...
import type { TreeViewElement } from "../modules/file-tree/types"

import type { ModelInfo } from "../store/editorStore"
import type { FilterFiles, MatchDetail, MatchResult, TreeMatchResult } from "./match"
...
export interface RenderedTreeItem extends Omit<Partial<TreeMatchResult & MatchDetail>, "kind"> {
  level: number
  kind: TreeViewElement["kind"] | "match"
}
...
export function readExcludeFileInRootDir(data: TreeViewElement[]): {
  ...
  const includePatterns = new Set<string>() // 用于存储包含规则

  if (data && data.length > 0) {
    const rootDirFile = data[0]
    const excludeFile = isDirectory(rootDirFile)
      ? rootDirFile.children?.find(
        child => child.name === ".gitignore" && child.kind === "file",
      )
      : null
 ...
 */
export function openedFileIds(models: ModelInfo[]): Set<string> {
...
// Simple path matching utility functions
...
function makeRe(pattern: string, options: { nocomment?: boolean, matchBase?: boolean } = {}): RegExp | null {
  try {
    const regexPattern = pattern
      .replace(/\./g, "\\.")
      .replace(/\*/g, ".*")
      .replace(/\?/g, ".")
    return new RegExp(`^${regexPattern}${options.matchBase ? "$" : ""}`)
  }
  catch {
    return null
  }
...
export function combinePatterns(
  ...
  if (filters?.includeFiles && filters?.includeFiles?.length > 0) {
    includePatterns = filters?.includeFiles
      .map((pattern) => {
        const regex = makeRe(pattern, { nocomment: true, matchBase: true })
