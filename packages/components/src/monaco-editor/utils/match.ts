...
import type { SearchOptions } from "../store/fileSearchStore"
import type { EmptyFileInterface } from "../types"
import { isDirectory, isFile } from "./filesystem"
import { combinePatterns, getBeforeAfter } from "./matchHelper"
...
// 树模式类型
export interface TreeMatchResult extends EmptyFileInterface {
...
function searchElements(
  ...
  for (const element of elements) {
    const currentPathParts = [...pathParts, element.name]
    ...
    else if (isDirectory(element) && element.children) {
      searchElements(
        element.children,
        currentPathParts,
        key,
...
function searchElementsToTree(
  ...
  const addTreeNode = (
    ...
    return {
      ...
      children: isDirectory(element) && element.children ? [] : undefined, // 仅当存在子目录时初始化 children 数组
    }
  }
