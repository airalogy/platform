...
import type { CellAttrs, TableRole } from "./utils"
import { fixTablesKey, removeColSpan, tableNodeTypes } from "@tiptap/pm/tables"
...
export function fixTable(
  ...
  if (hasFreeRows(table))
    tr = fixFreeRows(state, table, tablePos, tr)
  ...
  for (let i = 0; i < map.height; i++) {
    ...
    if (add > 0) {
      ...
      const nodes: Node[] = []
      for (let j = 0; j < add; j++) {
        const node = (tableNodeTypes(state.schema) as Record<TableRole, NodeType>)[role].createAndFill()
        if (node)
          nodes.push(node)
      }
...
export function hasFreeRows(table: Node): boolean {
  for (let i = 0; i < table.childCount; i++) {
    if (table.child(i).type.spec.tableRole === "row")
      return true
  }
  return false
}
