...
for (const mode of ["unclaimed", "qualified"] as const) {
  test(`installation UI ${mode}: exact identity, private-file rejection and acceptance history`, async ({ page, request }) => {
    ...
    try {
      ...
      if (mode === "qualified") {
        const qualifications = page.getByTestId("instrument-qualifications")
        await expect(qualifications).toContainText("Simulation passed; no physical qualification")
        await qualifications.getByRole("button", { name: "Revoke assessment" }).click()
        const revocation = page.getByRole("dialog").last()
        await revocation.locator("textarea").fill("Withdraw synthetic assessment, retain evidence")
        ...
        await expect(qualifications).toContainText("Assessment revoked")
      }
