...
| [Workspace interactions](./specs/workspace-interactions.spec.ts) | Responsive navigation and dialogs, keyboard access, DOI manual fallback, persistent preview/confirmation errors, Task next-step guidance and compact empty states. | Real import and Task persistence; selected provider/network failures and empty workbench responses are injected explicitly. |
