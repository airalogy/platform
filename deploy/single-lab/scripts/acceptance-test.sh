...
# Fresh deployment fixtures have no model provider. An empty model list is the
# intentional AI-off contract, not a GPT-only feature toggle.
jq -e '.ai_enabled == false and .enabled_chat_models == []' <<<"$status" >/dev/null || die "AI is unexpectedly enabled in the provider-free acceptance deployment"
