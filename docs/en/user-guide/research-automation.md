...
Aira may also place a pinned Protocol Run inside a mixed dependency graph. Planning the graph does not immediately assign laboratory work. Platform waits for every prerequisite, rechecks the pinned human Executor Binding, asks for the normal approval, and only then creates the Human Work Item. The assignee completes the existing Record form; the exact Protocol version, author, and Record are validated before downstream analysis or service work can begin. Other ready branches may continue in parallel, but the Run does not replan until the whole graph settles.
