...
The SDK now provides `output_contract.py` and `CaptureStore` in `output_capture.py`. This is a local library, **not yet an automatic runtime upload feature**. The library performs no adapter discovery, instrument method, network request, Record submission or DataAsset creation. Platform now has a separate receiving contract below. Existing structured-result-only commands retain their workflow; stop/result reconciliation is not removed.
...
Only a file-delivery-capable Gateway may request `file_delivery_version: airalogy.instrument-output-plan.v1` at lease time. The signed job then includes `file_outputs.plan` and `file_outputs.destination`. Missing capability fails before leasing or starting physical work; do not add this flag to an old client as a workaround. The bundled runtime does not yet implement automatic delivery.
...
Remaining integration: automatic SDK source collection, byte-pinned delivery/restart recovery, user-friendly file-review/Record selection and real workstation acceptance. The passing synthetic API/storage tests do not establish a complete instrument-file product or physical acceptance.
