...
The SDK now provides `output_contract.py` and `CaptureStore` in `output_capture.py`. This is a local library, **not yet a runtime upload feature**. No adapter discovery, instrument method, network request, Record submission or DataAsset creation occurs. Until scoped intake and durable asset registration are connected, the API refuses activation and new execution of managed commands declaring files (including optional files), rather than silently ignoring those outputs. Existing structured-result-only commands retain their workflow; stop/result reconciliation is not removed.
...
Remaining integration: bind the plan to API-authorized Project intake, acknowledge physical completion separately from file delivery, resume authenticated uploads, register private ResearchFiles and draft DataAssets with full installation provenance, and preview/confirm exact Record/sample associations. Only then may the declared-file execution gate be removed. No real instrument acceptance is claimed by local file fault tests.
