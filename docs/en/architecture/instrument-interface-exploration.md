...
This adds adaptive **selection among previously reviewed actions** to the [browser interface backend](./instrument-browser-interface.md). The local operator selects the application, semantic controls, observable states, literal actions and success checks first. Aira receives selected textual readbacks and chooses one action index, asks for missing information or proposes completion. The local runner independently checks every choice and the resulting state. It does not discover arbitrary software or invent new controls, parameter values, URLs or code.

This is development evidence, **not production equipment control or qualification**. Only isolated, reviewed simulation HTML supports fill/click. Live URL targets remain observation-only; loading a page/GET can still initialize equipment, so independent local authorization is required. No authenticated browser, native desktop, screenshot interpretation or real instrument has been qualified. Linux/macOS are the supported private-file runtime environments; Windows ACL support remains pending.
...
The installed package exposes `airalogy-interface-exploration` with the same arguments. The grant allows 1–5 model calls and 30–900 seconds. The selected browser definition has its own duration/step/observation limits; neither can extend the other. Each model response is bounded to 60 seconds and 16 KiB. These are call/time/output bounds, not a guaranteed financial ceiling.
...
Open work includes discovery of unknown native applications, authorized native/visual Computer Use, reviewed GUI adapters in managed production execution, supported-OS operations and a named real pilot with a responsible operator. These remain part of the full instrument-integration objective.
