...
The target plan is a versioned adaptive DAG supporting branches, parallel actions, cycles, retries, resource waits, and approvals. The current runtime supports both a bounded parallel frontier for two to four independent read-only Tool Actions and an acyclic dependency graph of two to eight read-only Tool Actions. Dependency edges are persisted, roots alone are initially released, each downstream node enters its own policy/approval boundary only after every parent completes, and a failed or rejected parent deterministically skips its descendants before replanning. A downstream node may bind one of its declared Tool arguments from a bounded structured-output path on a direct parent. Platform resolves the value only after that parent completes, records the source Action revision and output digest, reconstructs the downstream preview digest, and validates the complete input Schema before seeking approval or execution. Missing paths, out-of-range indexes, undeclared targets, or invalid resolved types fail closed and propagate through the graph. The Run remains at one durable graph barrier until all nodes settle, and the exact edge set is included in the result package. Cross-type dependency execution, parallel physical work, cycles, and arbitrary multi-agent graphs remain gated future work. Changes to the goal, success criteria, budget, or high-risk path create a new plan version and require renewed confirmation.
...
- independent advisory Reviewer Agent (delivered), bounded parallel and dependent read-only Tool graphs (delivered), cross-type dependency/multi-agent execution, and reproduction evaluation
