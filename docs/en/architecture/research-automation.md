...
- immutable Compute Environment catalog, scoped permissions, preview-confirm revisions, exact Research Environment pinning, governed Runner identity/readiness/environment bindings, approval/budget/lease/result Compute Job API, declared output ingestion, draft DataAsset registration, and an independently supervised reference Runner (delivered)
