...
- Instrument Gateway registration, governed command allowlists, signed job leasing, hardware-specific preflight interlock attestations, heartbeat, validated result receipt, and acknowledged remote stop (delivered); tiered closed-loop control remains future work
...
- independent advisory Reviewer Agent, formal human-finalized reproduction evaluation, bounded parallel and dependent read-only Tool graphs, and bounded mixed Protocol/structured Human Work/Tool/Resource/Instrument/External Service/Compute/Wait graphs (delivered); cycles and arbitrary multi-agent execution remain future work
