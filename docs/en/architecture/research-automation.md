...
Platform never runs research code in its API process or mounts a container-runtime socket into it. The runtime API is now implemented, but a compatible independently supervised Runner is still required for actual execution. This milestone returns structured JSON results; output-file upload and automatic DataAsset version registration remain a separate asset-ingestion boundary rather than accepting arbitrary filesystem paths from a Runner.
...
- immutable Compute Environment catalog, scoped permissions, preview-confirm revisions, exact Research Environment pinning, governed Runner identity/readiness/environment bindings, and approval/budget/lease/result Compute Job API (delivered); supervised reference Runner execution and output DataAsset registration remain future work
