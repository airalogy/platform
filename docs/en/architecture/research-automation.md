...
This milestone deliberately stops at catalog and Task pinning. Platform does not run research code in the API process, does not mount a container runtime socket into the API, and does not treat a pinned environment as an executable Aira path. Execution requires a separately authenticated, independently operated Compute Runner plus a governed Compute Job lease, approval, budget, input DataAsset, and result-registration state machine. Until that boundary is delivered, the UI labels the environment as a contract only and no Run can dispatch code through it.
...
- immutable Compute Environment catalog, scoped permissions, preview-confirm revisions, and exact Research Environment pinning (delivered); independent Runner, Compute Job, cost settlement, and DataAsset result registration remain future work
