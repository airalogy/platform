...
This milestone deliberately stops before Compute Job execution. Platform now governs the environment catalog, Task pins, Runner identity/readiness, and exact Runner-to-environment authorization, but does not run research code in the API process, mount a container runtime socket into the API, or treat a pinned environment as an executable Aira path. Execution still requires the independent Runner process plus a governed Compute Job lease, approval, budget, input DataAsset, and result-registration state machine. Until that boundary is delivered, the UI labels the environment as a contract only and no Run can dispatch code through it.
...
- immutable Compute Environment catalog, scoped permissions, preview-confirm revisions, exact Research Environment pinning, and governed Runner identity/readiness/environment bindings (delivered); Compute Job leases, isolated runtime execution, cost settlement, and DataAsset result registration remain future work
