...
Instrument integration progresses through data import, guided execution, device-confirmed assisted control, and policy-bounded closed-loop automation. Aira never sends arbitrary commands directly to equipment. A local Instrument Gateway accepts signed, structured, allowlisted jobs and provides state checks, audit, and governed stop requests.
