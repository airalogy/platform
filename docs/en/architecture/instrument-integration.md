...
| Autonomous instrument software discovery/launch/exploration | Explicitly selected macOS startup has a separate expiring local preview/confirmation and single-use receipt; [inspect/windows](./instrument-native-interface.md) provide bounded metadata. Autonomous discovery and vendor-action qualification remain open |
