...
| Instrument software discovery/launch/exploration | [Scoped macOS discovery](./instrument-native-interface.md#find-software-in-a-selected-directory) lists metadata after directory/depth confirmation, then feeds independent `inspect`. Selected startup has its own expiring approval and single-use receipt; `windows` supplies bounded metadata. Intent-driven software selection, vendor-action qualification and cross-platform backends remain open |
