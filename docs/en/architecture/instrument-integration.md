...
| Record browser demonstration and reuse | [Owned offline browser input recording](./instrument-interface-workflow.md#browser-demonstration), selected state/action evidence, distinct human-event lineage, fixed export and the same installed adapter/Job recovery. No native/visual recording or automatic parameter generalization; visible human and real-device acceptance remain separate |
| Save and reuse successful exploration | [Reviewed fixed local workflows](./instrument-interface-workflow.md) retain actual steps, initial conditions, original success checks and lineage; deterministic replay needs fresh approval, not another model call. Not managed GUI execution or hardware qualification |
| Installed adapter to browser workflow | [Byte-pinned local process bridge](./instrument-interface-worker.md), source-included owned reference, independent installs and actual Instrument Job/receipt recovery. No remote shell, new control endpoint or physical/native/visual qualification |
| Installed adapter to native read definition | [Separate macOS read worker](./instrument-interface-worker.md#installed-native-read-adapters), existing exact-process/window/privacy checks and an owned source reference; real-build/no-launch checks and opt-in graphical installed/API acceptance. No startup, UI writes, screenshots or vendor qualification |
...
| Reviewed HTTP JSON control | [Separate fixed-operation control backend](./instrument-http-control.md), correlated stateful loopback reference, independent package tests and installed Job/receipt recovery; not real-equipment qualification |
