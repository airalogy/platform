...
| Native accessibility and visual control backends | [macOS native observation](./instrument-native-interface.md) supports pinned, explicitly selected running applications, private surveys, Aira/manual review and ordinary read-only definitions. Native writes, Windows/Linux and visual control remain unimplemented; a real target is still required for qualification |
