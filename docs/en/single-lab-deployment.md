...
SMS login is disabled by default with `SMS_LOGIN_ENABLED=false`, so Single-Lab does not depend on an SMS service. To enable it, configure the Alibaba Cloud access key fields and a non-empty `SMS_COUNTRY_CODE_ALLOWLIST`. Allowing country code `86` also requires `ALIBABA_CLOUD_SMS_SIGN_NAME` and `ALIBABA_CLOUD_SMS_VERIFY_CODE_TEMPLATE_CODE`; allowing any other country code requires `ALIBABA_CLOUD_SMS_SENDER_ID`. Then set `SMS_LOGIN_ENABLED=true` and restart the API container. The login page reads the effective capability from `/api/instance`, so later switch changes do not require rebuilding Web. The first Platform deployment containing this feature must rebuild Web because the login-page code itself has changed. This switch affects SMS login only; it does not disable SMS used for password reset, phone changes, or other verification flows.
