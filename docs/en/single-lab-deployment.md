...
AI features are optional. The instance auto-detects a usable provider; set `AI_ENABLED=false` to hide all AI entry points and reject AI requests explicitly. Protocol and Record workflows remain available without AI. Setting `AI_ENABLED=true` still requires a configured provider. GPT is disabled by default: it is omitted from the model selector and rejected by the API. To enable it, set both `OPENAI_API_KEY` and `ENABLE_GPT_MODEL=true`. A legacy external Masterbrain deployment may instead use `MASTERBRAIN_CALL_MODE=external` with `CHAT_API_ENDPOINT`. This backend-only change requires an API container restart, not a Web image rebuild.

Research work and approval requests always appear in the private in-product attention inbox. Optional email reminders are disabled by default. To enable them, configure `SMTP_HOST`, `SMTP_PORT`, `SMTP_FROM_ADDRESS`, optional paired `SMTP_USERNAME`/`SMTP_PASSWORD`, and `SMTP_SECURITY=starttls` or `ssl`, then set `RESEARCH_EMAIL_NOTIFICATIONS_ENABLED=true` and restart the API. Remote production deployments reject unencrypted SMTP. Delivery uses the persistent job worker with five attempts; a failure is reported in the inbox but never blocks the underlying Research Task. `SITE_URL` supplies the same-origin task link included in the email.

SMS login and mandatory phone verification at signup are disabled by default with `SMS_LOGIN_ENABLED=false` and `SMS_SIGNUP_REQUIRED=false`, so Single-Lab does not depend on an SMS service. To enable either capability, configure the Alibaba Cloud access key fields and a non-empty `SMS_COUNTRY_CODE_ALLOWLIST`. Allowing country code `86` also requires `ALIBABA_CLOUD_SMS_SIGN_NAME` and `ALIBABA_CLOUD_SMS_VERIFY_CODE_TEMPLATE_CODE`; allowing any other country code requires `ALIBABA_CLOUD_SMS_SENDER_ID`. Then enable the required switch and restart the API container. The login and signup pages read the effective capabilities from `/api/instance`, so later switch changes do not require rebuilding Web. The first Platform deployment containing these capability-aware pages must rebuild Web because their code changed. The switches remain independent and do not disable SMS used for password reset or phone changes.
