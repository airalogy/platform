...
Community 部署默认可以通过邮箱和密码创建账号。手机号验证和短信发送是部署方按需启用的实例能力，不是本地启动和注册账号的前置条件。
...
修改此后端配置后需重启 API。本功能首次部署时需要重新构建包含能力判断代码的 Web；此后仅切换配置值不需要再次构建 Web。该开关只控制短信登录，不会直接禁用短信重置密码、修改手机号或其他验证码用途。
