...
目标计划是可版本化的自适应 DAG，而不是只能向前的页面步骤。当前运行时同时支持两种有边界的数字执行：2–4 个相互独立的只读 Tool Action 并行前沿，以及 2–8 个只读 Tool Action 组成的无环依赖图。依赖边持久化保存，初始只释放根节点；下游节点仅在全部前置完成后才进入自己的策略/审批边界，任一前置失败或被拒绝会确定性跳过其后代。Run 会停留在一个持久图屏障上，直到全部节点结束，准确边集也会进入结果包。跨类型依赖执行、并行物理工作、循环和任意多 Agent 图仍是受控的后续能力。修改目标、成功标准、预算或高风险路径必须产生新的计划版本并重新确认。
