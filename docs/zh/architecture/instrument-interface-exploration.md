...
成功且完整关闭的本地探索证据可以[保存为经审核的固定流程](./instrument-interface-workflow.md)，后续重放无需再次调用模型。导出仅保留实际步骤、初始条件、原定成功标准和来源，不导入、安装或启用适配包；中止/不确定记录不能转为成功流程。新执行仍遵守原后端的目标、确认和模拟范围限制。
