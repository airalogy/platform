...
在**实验室 → 资源库 → 仪器网关 → 选择网关 → 准备适配包 → 查找或导入适配包**中，Owner/Manager 可以选择已在本地测试的 ZIP，预览指令、来源及文件摘要，确认准确内容和实验室保存位置。未选择网关时也可访问实验室适配包。API 不导入、不构建、不执行驱动代码。文件对实验室成员可读，不公开；不得打包凭据、工作站配置密钥或未经许可的客户资料。
...
新建适配器之前，可选择**查找可复用适配包**。填写准确厂商和型号，再按需补充固件、控制软件及版本、操作系统、处理器架构和 Gateway/Python 版本；未知可选信息留空。名称去掉首尾空格后按区分大小写的字面值比较，不模糊匹配、不解释别名或版本范围。这只读取实验室私有目录，不探测设备、已安装软件或公共全局目录，也不调用模型。

受实验室权限保护的 `POST /instrument-adapter-packages/match?lab_id=...` 接收 `profile` 和严格布尔值 `include_revoked`。Gateway 使用准确 SemVer，Python 使用准确主次版本。数据库在分页前筛选同一声明行中的厂商和型号；每页默认 10 个包、最多 20 个，返回 `has_more` 和 `next_offset`。每个匹配组合独立比较，不把一行的固件和另一行的软件拼成虚构兼容组合。

- **声明匹配**：九项已提供信息匹配同一组合及其运行时列表，不等于设备已验收。
- **需要补充信息或核对**：目标信息缺失，或包使用 `any`、`all`、`unknown`、`unspecified`、`*` 等宽泛/未知声明；这些词不作为可执行通配符。
- **兼容声明冲突**：至少一项已提供信息与该组合的字面声明不同。

结果展示归档身份、来源审核快照、逐字段差异和包内测试引用。即使声明做过非模拟测试，此处也未独立核查；`qualification_checked`、`hardware_authorized`、`installation_authorized` 均为 false。默认不显示撤销版本，可显式包含以供检查。修改目标或重新打开查找会清除旧结果。

**检查与审核**通过受范围保护的 `GET /instrument-adapter-packages/{release_id}` 重新读取准确版本。查找不自动选包、建立安装授权或绕过后续撤销。继续沿现有受保护下载、来源审核、具体设备验收及安装流程进行。没有匹配只说明本实验室声明目录没有结果，不表示适配器不可能存在；可导入另行准备的包，或进入有界源码生成。AI 关闭时仍可完整使用，无需新增迁移。
