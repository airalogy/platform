...
此功能不安装或启用适配包、不提交 Record、不发布 Knowledge，也不会把流程公开。编辑定义、步骤、初始条件或成功标准会形成不同摘要，需要重新审核；重新计算摘要不是验收。生产复用仍须经过适配包版本审核、安装、目标准入及 Instrument Job 的权限、预约、本地确认、租约、停止/核对和数据回传规则。人工示范录制/泛化、受治理 GUI 执行、支持系统验收和明确的真实试点仍待完成。
