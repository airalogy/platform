...
具备科研计算使用权的 Task 创建者可以在创建 Task 时选择已启用环境。Platform 会把准确修订复制到 Research Environment。这不会执行代码、预留预算，也不表示存在合资格 Compute Runner。当前里程碑只提供治理、可复现版本固定与 Runner 授权；在 Compute Job 生命周期和独立 Runner 进程交付前，纯计算环境 Task 仍由人工控制，Task 页面也会明确说明暂不可执行。
...
Platform 会在 API 层创建每个新的人工或 Aira Protocol、Tool、Wait、Resource、Instrument 或 External Service Action 前检查限额，拒绝过期预览和超限写入。确认的预算记录耗尽上限时会立即暂停 Run；从其他入口发现截止时间或预算到达边界时，会在下一个运行边界安全暂停。已批准 External Service Job 会自动预留准确报价并结算已确认实际费用；模型、计算及其他尚未集成的成本仍须显式登记。
