...
Platform 每次只释放一步。只有上一个 Instrument Job 返回符合 Schema 的结果，且权限、固定项、Binding、预约状态和上限仍全部通过时，系统才会创建下一步。后续高风险步骤会显示“复核并继续”，不能被首次整体确认概括授权。会话活动或待复核时，同一控件可“停止控制会话”。指令过期、比较无效、达到上限、遇到明确暂停目标、失败或停止时，Platform 都不会猜测或重试；工作台会保留准确会话状态和原因，等待人工复核。构建器和所有控制在 AI 关闭时仍完可用。
