...
Lab 计算管理员可以在“实验室资源库 → 计算环境”中新增 Runner、仅一次获取其凭据，并设置硬性并发上限。独立 Runner 进程安装后，只有在报告非 root 执行、只读根文件系统、网络隔离和无宿主机挂载四项控制时才显示为已就绪。必须逐一把 Runner 绑定到允许执行的准确环境修订；新修订需要重新审核和绑定。停用 Runner 或移除绑定会停止后续工作，但不会删除审计历史。仅登记和绑定仍不会执行任何代码。
...
确认只创建审批，不会直接执行代码。获授权人员批准准确影响后，Platform 才会为有预算的 Task 预留最大成本并排队。一个受独立监管、与协议兼容且明确绑定该环境修订的 Runner 必须在线；它会主动领取签名短租约，只下载固定输入，在 Platform API 进程之外运行，并回报心跳、结果、申报用量或失败。Platform 按结果 Schema 和硬性用量上限校验，依据实测墙钟时间结算实际成本，并保留完整状态轨迹。需要停止时使用“取消计算”并填写原因；已经交付的作业只有在 Runner 确认后才真正取消，恢复暂停 Task 前必须检查部分输出。
