...
- **科研工具**：运行白名单中且锁定版本的数字能力，例如检索已审核 Knowledge 或可选的 Literature Provider。输入和输出都会经过 Schema 校验，执行有超时、重试和完整留痕。当多个只读检索确实彼此独立时，Aira 可创建包含 2–4 个 Action 的有界并行前沿；每个分支保留自己的审批和审计记录，只有所有分支都完成、失败或被拒绝后才会重新规划。
