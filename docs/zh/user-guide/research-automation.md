...
选中 Gateway 后，只添加它确实可执行的指令。每条指令必须选择一个有效的具体设备 Resource，固定当前修订，并定义带版本的标识、输入/结果 JSON Schema、风险、超时与设备端确认要求。中高风险指令必须在物理设备端确认。Schema 变更会生成新的允许清单审计修订，已创建作业继续使用原契约。停用指令或 Gateway 即可阻止之后的派发。注册本身不会启动设备，也不会向 Aira 授予直接设备权限；后续 Instrument Job 执行层必须使用该允许清单。
