...
single-Lab 默认启用 `LAB_STRUCTURE_MODE=structured`。一个 Lab 内可建立最多三级团队树，成员可同时属于多个团队，父团队管理员可以维护下级团队。团队只表达组织归属，Project 与 Protocol 仍构成独立的资源层级；两者通过作用域授权建立多对多关系。

权限工作台提供固定角色、Lab / Project / Protocol 三种作用域、向下继承、资源继承断点、授权到期时间、撤销和有效权限解释。用户的最终能力是所有直接授权、所属团队及其上级团队授权、资源上级授权和兼容旧角色的并集。系统不提供任意策略表达式或显式拒绝规则，避免出现难以解释的冲突策略。

授权人只能下放自己在目标作用域内已经拥有的能力。Lab owner 和 administrator 是成员身份，只能在成员管理中设置，不能当作普通作用域授权发放。Project manager 可在自己管理的 Project 范围内下放较低角色；团队管理员只能维护本团队及下级团队成员，不能因此获得研究资源权限。所有授权创建、修改、撤销以及成员移出 Lab 导致的撤销都会写入追加式审计日志。

从简单成员表升级时无需一次性迁移：原有 `ProjectUser`、Lab Group、`ProtocolUser` 和 Project Group 角色仍会进入同一个有效权限解释器。新部署应优先使用“团队 + 作用域授权”，不要再为同一管理目的同时维护旧 Group 和新 Team。
