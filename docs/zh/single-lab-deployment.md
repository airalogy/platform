...
它不是高可用集群，也不是企业身份系统。SSO/LDAP、多节点故障切换、离线安装包和特定合规控制不属于这个 profile 的范围。
...
single-Lab 默认启用 `LAB_STRUCTURE_MODE=structured`。一个 Lab 内可建立最多三级组织单元树，组织单元可标记为部门、课题组、公共平台、项目团队、委员会或其他类型。成员可同时属于多个组织单元，上级单元管理员可维护下级单元。组织单元只表达组织归属，Project 与 Protocol 仍构成独立的资源层级；两者通过作用域授权建立多对多关系。

权限工作台提供固定角色、Lab / Project / Protocol 三种作用域、向下继承、资源继承断点、授权到期时间、撤销和有效权限解释。用户的最终能力是所有直接授权、所属组织单元及其上级单元授权、资源上级授权和兼容旧角色的并集。系统不提供任意策略表达式或显式拒绝规则，避免出现难以解释的冲突策略。

授权人只能下放自己在目标作用域内已经拥有的能力。Lab owner 和 administrator 是成员身份，只能在成员管理中设置，不能当作普通作用域授权发放。Project manager 可在自己管理的 Project 范围内下放较低角色；组织单元管理员只能维护本单元及下级单元成员，不能因此获得研究资源权限。所有授权创建、修改、撤销以及成员移出 Lab 导致的撤销都会写入追加式审计日志。

从简单成员表升级时无需一次性迁移：原有 `ProjectUser`、Lab Group、`ProtocolUser` 和 Project Group 角色仍会进入同一个有效权限解释器。升级前已有的 Group 会作为 `other` 类型的组织单元保留；新部署应优先使用“组织单元 + 作用域授权”。

完整的角色能力矩阵、继承规则、管理操作、API 和排错说明见 [Lab 权限管理](./access-control.md)。
...
Research 工作与审批请求始终进入产品内的私有待处理通知。邮件提醒默认关闭；如需启用，应配置 `SMTP_HOST`、`SMTP_PORT`、`SMTP_FROM_ADDRESS`、可选但必须成对设置的 `SMTP_USERNAME`/`SMTP_PASSWORD`，以及 `SMTP_SECURITY=starttls` 或 `ssl`，再设置 `RESEARCH_EMAIL_NOTIFICATIONS_ENABLED=true` 并重启 API。远程生产部署拒绝无加密 SMTP。邮件由持久化工作器最多尝试五次；失败会在站内显示，但绝不阻塞原 Research Task。邮件中的同源任务链接来自 `SITE_URL`。

Single-Lab 默认设置 `SMS_LOGIN_ENABLED=false` 和 `SMS_SIGNUP_REQUIRED=false`，因此短信登录与注册时强制验证手机号都关闭，不依赖短信服务。若需启用任一能力，应配置阿里云 access key 和非空的 `SMS_COUNTRY_CODE_ALLOWLIST`；allowlist 包含中国区 `86` 时，还须配置 `ALIBABA_CLOUD_SMS_SIGN_NAME` 与 `ALIBABA_CLOUD_SMS_VERIFY_CODE_TEMPLATE_CODE`，包含其他国家或地区代码时还须配置 `ALIBABA_CLOUD_SMS_SENDER_ID`。配置完整后启用相应开关并重启 API 容器。登录页和注册页都从 `/api/instance` 读取最终能力，因此本功能首次部署时需因页面代码变化重新构建 Web；之后只切换配置值无需重建 Web。两个开关相互独立，也不会禁用短信重置密码或修改手机号。
