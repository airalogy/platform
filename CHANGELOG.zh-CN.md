...
- 接入 Renderer 列选择器原生的“显示全部列”和“恢复默认列”，同时覆盖协议字段和提交信息；保留按用户/Protocol 的偏好，约束窄屏菜单并支持横向滚动，不更改导出范围。上游正式发布前，以锁定版本的 pnpm 补丁复用同一组件实现，Platform 不重复遍历字段，也不修改 Python 包。
