...
- 新增实验室/设备范围的 GUI 接入重放工作台：支持手工 JSON 导入导出、经资料处理确认的可选 Aira 草拟、确定性预览确认、修订审计和离线重放工具；提供真实浏览器操作的合成仪器软件示例及失败关闭的观察校验。重放不会安装驱动、打开真实仪器软件或授予实机控制；原生/视觉 Computer Use、设备配对、安装与实机验收仍为 RFC #5 待办。
