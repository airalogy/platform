...
- 新增可持久化的 Research Task 与 Run，并在产品内提供任务与指派工作台：固定版本化 Research Environment，使用类型明确的 Protocol Action、经过校验的 Record 表单交付与回传、仅追加的来源事件；Aira 可在服务重启后安全续跑，AI 不可用时仍可手工执行，科研结论必须由人明确审核。
