...
- 接通 POSIX Gateway 的仪器文件自动交付：经审核适配器返回采集时文件摘要，运行端先持久化原件再确认执行完成，采集期间保持租约；回执丢失后仅恢复固定目标的文件交付，不重复实验、不初始化驱动。受管理启动绑定显式输出目录与已安装文件声明；字节、范围或资产回执不匹配时保留恢复日志。文件仍为待审核草稿资产。已通过模拟文件、安装后独立进程与真实 API/存储集成验证，不代表实机验收；专用文件审核界面和工作站认证仍待完成。
