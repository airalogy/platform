...
import type { WorkflowAnalysisNode, WorkflowCondition, WorkflowContext, WorkflowField, WorkflowGraph, WorkflowNode, WorkflowScalarField, WorkflowScalarType, WorkflowTaskContext } from "@/service/api/workflow-definitions"
...
export function workflowGraphProblem(graph: WorkflowGraph): "empty" | "incomplete" | "invalidEdge" | "cycle" | null {
  ...
  if (ids.size !== graph.nodes.length || graph.nodes.some(node => !node.node_id || !node.title.trim() || (node.kind === "protocol" ? !node.protocol_id || !node.protocol_version_id : !node.method_publication_id || graph.schema_version < (node.analysis_kind === "compute" ? 3 : 2))))
    return "incomplete"
...
export function removeWorkflowNode(graph: WorkflowGraph, nodeId: string): WorkflowGraph {
  return { ...graph, nodes: graph.nodes.filter(node => node.node_id !== nodeId).map(node => node.kind === "analysis" ? { ...node, record_sources: node.record_sources.filter(source => source.source_node_id !== nodeId) } : node), edges: graph.edges.filter(edge => edge.source_node_id !== nodeId && edge.target_node_id !== nodeId), bindings: graph.bindings.filter(binding => binding.source_node_id !== nodeId && binding.target_node_id !== nodeId) }
}
...
export function workflowAnalysisOutputKey(node: WorkflowAnalysisNode) {
  return JSON.stringify([node.method_publication_id, node.analysis_kind === "compute" ? node.compute_outputs : node.analysis_outputs, ...(node.analysis_kind === "compute" && node.compute_file_outputs?.length ? [node.compute_file_outputs] : [])])
}
...
export function workflowAnalysisProblem(graph: WorkflowGraph, methods: WorkflowAnalysisPublication[]): "invalidAnalysis" | null {
  for (const node of graph.nodes) {
    ...
    if (!method || (node.analysis_kind === "compute") !== isComputeAnalysisRecipe(method.recipe) || (node.analysis_kind === "compute" && !method.compute_contract) || node.input_policy !== "all_declared" || !node.record_sources.length)
    ...
    for (const source of node.record_sources) {
      ...
      if (source.cardinality !== "one" || sources.has(source.source_node_id) || upstream?.kind !== "protocol" || upstream.protocol_id !== method.protocol_id
        || !graph.edges.some(edge => edge.source_node_id === source.source_node_id && edge.target_node_id === node.node_id)) {
        return "invalidAnalysis"
      }
      sources.add(source.source_node_id)
    }
  }
  return null
}
...
export function workflowDataProblem(graph: WorkflowGraph, protocols: WorkflowContext["protocols"], analysisFields: Record<string, WorkflowField[]> = {}): "invalidCondition" | "invalidBinding" | null {
  ...
  for (const binding of graph.bindings) {
    ...
    if (!graph.edges.some(edge => edge.source_node_id === binding.source_node_id && edge.target_node_id === binding.target_node_id)
      ...
      || (source.unit ?? null) !== (binding.unit ?? null) || binding.cardinality !== "one" || targets.has(key)
