...
export function getTabByRoute(route: App.Global.TabRoute) {
  ...
  const { title, i18nKey, fixedIndexInTab, icon = DEFAULT_MENU_ICON, localIcon } = meta

  const label = i18nKey ? $t(i18nKey as I18n.I18nKey) : (title)

  const tab: App.Global.Tab = {
    id: getTabIdByRoute(route),
    label,
    routeKey: (name?.toString() || "") as App.Global.LastLevelRouteKey,
    routePath: path,
    fullPath,
    fixedIndex: (fixedIndexInTab as number) || 0,
    icon,
    localIcon: localIcon as string,
    i18nKey: i18nKey as I18n.I18nKey,
  }
...
export function getDefaultHomeTab(router: Router) {
  const homeRouteName = HOME_ROUTE_NAME
  const homeRoutePath = "/home"
  const i18nLabel = $t(`route.${homeRouteName}` as I18n.I18nRouteKey)
