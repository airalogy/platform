...
export interface ILLM extends LLMOptions {
  ...
  templateMessages?: (messages: ChatMessageItem[]) => string
  ...
  streamChat: (
    ...
    options?: LLMFullCompletionOptions,
  ) => AsyncGenerator<ChatMessageItem, PromptLog>
  jsonComplete: (
    prompt: string,
    options?: LLMFullCompletionOptions,
  ) => AsyncGenerator<string, PromptLog>
  jsonChat: (
    messages: ChatMessageItem[],
    options?: LLMFullCompletionOptions,
  ) => AsyncGenerator<ChatMessageItem<any>, PromptLog>
...
export interface LoadSubmenuItemsArgs {
  config: Partial<ExtraContextConfig>
  eventBus: UseEventBusReturn<any, any>
  eventName?: string
  fetch?: FetchFunction
}
...
export interface IDE {
  ...
  getWorkspaceConfigs: () => Promise<AiralogyRcJson[]>
  fileExists: (filepath: string) => Promise<boolean>
  writeFile: (path: string, contents: string) => Promise<void>
  showVirtualFile: (title: string, contents: string) => Promise<void>
  getAiralogyDir: () => Promise<string>
  getAiralogyResearchProtocols: () => Promise<Airalogy.ResearchProtocol.PackageItem[]>
  openFile: (path: string) => Promise<void>
  openRNMDFile: (path: string) => Promise<void>
  ...
  getAiralogyAuthToken: () => Promise<string | undefined>
...
export type AiralogyRcJson = Partial<SerializedContinueConfig> & {
  mergeBehavior: ConfigMergeType
}

// config.ts - give users simplified interfaces
export interface Config {
  /** If set to true, Continue will collect anonymous usage data to improve the product. If set to false, we will collect nothing. Read here to learn more: https://docs.airalogy.dev/telemetry */
  allowAnonymousTelemetry?: boolean
  /**
...
export interface ExtraContextConfig {
  allowAnonymousTelemetry?: boolean
  models?: ILLM[]
  systemMessage?: string
  completionOptions?: BaseCompletionOptions
