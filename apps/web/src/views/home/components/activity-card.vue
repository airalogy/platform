  ...
  >
    <template #header>
      <div class="flex items-center">
        ...
        <div v-else class="aira-type-body ml-4">
          <div class="-mb-1">
            <router-link class="align-middle no-underline hover:router-link" :to="userRoute">
              {{ props.user.name || props.user.username }}
            </router-link>
            <span class="ml-3 inline-block align-middle">{{ props.activity }}</span>
          </div>
    ...
    <div v-else class="aira-type-label my-2 w-fit">
      <router-link v-if="activityRoute.lab" :to="activityRoute.lab" class="hover:router-link">
        {{ props.lab.name }}
      </router-link>
      <template v-if="activityRoute.project">
        <span class="aira-type-meta aira-text-subtle pointer-events-none mx-1 inline-block -mt-1"
        >/</span
        >
...
const activityRoute = computed<
  Record<"project" | "lab" | "protocol" | "record", RouteLocationRaw | null>
>(() => {
  ...
  return {
    ...
    protocol:
      protocol?.id && lab?.uid && project?.uid
        ? {
            name: "protocol-info",
            params: { labUid: lab.uid, projectUid: project.uid, protocolUid: protocol.uid },
          }
        : null,
    record:
      record?.record_id && lab?.uid && project?.uid
        ? {
            name: "protocol-record-report",
            params: {
              labUid: lab.uid,
              projectUid: project.uid,
              protocolUid: record.metadata?.airalogy_protocol_id,
