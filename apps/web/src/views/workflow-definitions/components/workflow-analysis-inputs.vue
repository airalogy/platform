<template>
  <section class="analysis-inputs" data-testid="workflow-analysis-settings">
    <workflow-project-analysis-inputs v-if="node.analysis_kind === 'project'" :graph="graph" :node="node" :method="method" :disabled="disabled" @sources="emit('sources', $event)" @outputs="emit('projectOutputs', $event)" />
    <template v-else>
...
import WorkflowProjectAnalysisInputs from "./workflow-project-analysis-inputs.vue"
...
const emit = defineEmits<{ sources: [sources: WorkflowAnalysisNode["record_sources"]], outputs: [outputs: WorkflowAnalysisOutput[]], computeOutputs: [outputs: WorkflowComputeOutput[]], computeFiles: [outputs: WorkflowComputeFileOutput[]], projectOutputs: [outputs: WorkflowProjectOutput[]] }>()
const { t } = useI18n()
const outputId = ref("")
...
const builtinRecipe = computed(() => props.method && !isComputeAnalysisRecipe(props.method.recipe) && !isWorkflowProjectRecipe(props.method.recipe) ? props.method.recipe : null)
...
const draftOutput = computed<WorkflowAnalysisOutput | null>(() => {
  if (props.node.analysis_kind === "compute" || props.node.analysis_kind === "project" || !/^[a-z][a-z0-9_-]{0,63}$/.test(outputId.value) || !fieldKey.value || props.node.analysis_outputs.some(output => output.output_id === outputId.value))
    return null
...
function addOutput() {
  if (!draftOutput.value || props.disabled || props.node.analysis_kind === "compute" || props.node.analysis_kind === "project")
    return
