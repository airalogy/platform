...
def generation_prompt(spec, previous=None):
    return "\n".join(
        [
            ...
            "For independently documented JSON control APIs, the trusted SDK provides http_control.HttpControlClient.from_file(config_path, operations) and HttpControlOperation(method, path, body_fields=tuple(), query_fields=tuple(), max_request_bytes=65536, max_response_bytes=65536, statuses=(200,)). client.call(operation, body=None, query=None, timeout_seconds=5, stop_event=event) fixes GET/POST/PUT in reviewed code and never retries. Separate airalogy.http-control-config.v1 privately pins origin/IP/authentication and enabled_operations; read-only config does not enable control. Success status 202 is request acceptance only. Bind device operation identifiers to the exact job, independently read back parameters/completion and implement documented stopping; missing vendor semantics remain questions. Use fake transports in the fixed offline tests, never a real device or service.",
            "For separately reviewed fixed browser workflows, the trusted SDK provides airalogy_instrument_gateway.interface_process.InterfaceProcessClient.from_file(config_path); native macOS observation uses the distinct NativeReadProcessClient.from_file(config_path). Both expose call('probe', stop_event=event) and call('execute', job_id=job.job_id, timeout_seconds=bounded_seconds, stop_event=event), returning data and the fixed workflow_digest or definition_digest, respectively. Execution additionally correlates the exact Job UUID. The separately prepared private runtime fixes all executable, dependency, target and workflow/definition identities; do not generate paths, shell commands, browser selectors, native actions or runtime configuration in adapter source. The native read backend does not launch, focus, click, fill or grant Accessibility permission. Browser actions support owned simulation HTML only. Implement preflight(job) for the fixed interlocks without inventing operator presence or emergency-stop capability; interlock observations are not physical safety proof. Forward cancellation, preserve uncertainty and never retry a worker operation or claim that terminating a process physically stops equipment. Fixed offline tests use injected fake clients only: no browser, desktop, worker subprocess or hardware in source development. A readback is selected text, not scientific validation. Missing target, completion or stopping semantics require questions, not implicit authority or fabricated data.",
