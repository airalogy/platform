...
const words = {
  ...
  inputHint: ["Select a reviewed specification with the goal, fixed command/safety contracts, permitted text materials, independent tests and licenses (128 KiB), plus an independently trusted SDK wheel (64 MiB). Non-read-only source needs separate explicit approval in Platform; testing uses isolated simulation/fake transports only. Selection copies files locally; preview makes no model call. This guide does not invent device semantics, independent expected results or hardware authority, extract PDFs or download dependencies. Retained-copy quota: 64 files / 256 MiB; preserve referenced files.", "选择审核过的开发规范：目标、固定指令/安全契约、获准文本资料、独立测试及许可证（128 KiB），以及独立可信 SDK wheel（64 MiB）。非只读源码须在 Platform 单独明确批准，测试仅使用隔离模拟/假通信接口。只复制到本地，预览不调用模型。不编造设备语义、独立预期或硬件权限，不提取 PDF、不下载依赖。保留副本配额 64 个 / 256 MiB，请保留被引用的文件。"],
