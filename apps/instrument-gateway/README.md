...
An adapter subclasses `InstrumentAdapter` and implements exact-version `supports`, device-local `confirm`, blocking `execute`, and idempotent `safe_stop`. Export a factory accepting an optional configuration `Path`:
