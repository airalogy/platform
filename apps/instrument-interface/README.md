...
`pnpm gateway:native` (installed: `airalogy-interface-native`) adds a **read-only macOS Accessibility backend** for explicitly selected, already-running applications. Build/doctor/prepare never open vendor software; capture pins bundle integrity, process lifetime and one exact window, omits passwords/private regions and defaults to no input values. Reuse `gateway:survey run/assemble` and the Platform Aira review panel, then separately preview/confirm a native read definition. No native clicks, fills, app discovery/launch, screenshot, visual fallback or hardware authorization. See the bilingual **Native macOS interface observation** guide for toolchain, privacy, same-user trust and real AppKit acceptance. Windows/Linux native support remains pending.
...
See the repository's bilingual **Browser interface backend** guide for the precise browser contract and privacy/network limits. Windows private-file ACL, native writes and visual/production GUI control are not implemented; synthetic CI does not qualify an instrument.
